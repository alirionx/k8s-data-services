...
output "service_internal_ip" {
  ...
  value = data.kubernetes_resources.minio_service.objects[0].spec.clusterIP
}
output "service_port_api" {
  depends_on = [data.kubernetes_resources.minio_service]
  value = data.kubernetes_resources.minio_service.objects[0].spec.ports[0].port
}
output "service_port_console" {
  depends_on = [data.kubernetes_resources.minio_service]
  value = data.kubernetes_resources.minio_service.objects[0].spec.ports[1].port
}
output "service_node_port_api" {
  depends_on = [data.kubernetes_resources.minio_service]
  value = var.service_endpoint_type == "NodePort" ? data.kubernetes_resources.minio_service.objects[0].spec.ports[0].nodePort : ""
}
output "service_node_port_console" {
  depends_on = [data.kubernetes_resources.minio_service]
  value = var.service_endpoint_type == "NodePort" ? data.kubernetes_resources.minio_service.objects[0].spec.ports[1].nodePort : ""
}
output "service_internal_dns" {
  depends_on = [data.kubernetes_resources.minio_service]
  value = "minio-${var.service_id}.${var.service_namespace}"
}
output "service_external_ip" {
  depends_on = [data.kubernetes_resources.minio_service]
  value = var.service_endpoint_type == "LoadBalancer" ? data.kubernetes_resources.minio_service.objects[0].status.loadBalancer.ingress[0].ip: ""
}
  ...
  value = jsonencode( 
    {
      ...
      port_api: data.kubernetes_resources.minio_service.objects[0].spec.ports[0].port,
      port_console: data.kubernetes_resources.minio_service.objects[0].spec.ports[1].port,
